<template>
  <div class="page">
    ...
    <div class="grid">
      <ResourceCard v-for="item in spots" :key="item.id" :item="item" type="SCENIC" @favorite="favorite('SCENIC', $event)" />
    </div>
...
async function load() {
  ...
  spots.value = await http.get('/scenic-spots', { params })
}

async function favorite(type, item) {
